...
public class HeaderSpanSizeLookup extends GridLayoutManager.SpanSizeLookup {
    ...
    public HeaderSpanSizeLookup(MultipleHeaderAdapter multipleHeaderAdapter, int spanCount) {
        setSpanIndexCacheEnabled(true);
        mMultipleHeaderAdapter = multipleHeaderAdapter;
        mSpanCount = spanCount;
    }

    @Override
    public int getSpanSize(int position) {
        if (mMultipleHeaderAdapter.isHeaderView(position)) {
            return getHeaderSpanSize(position);
        } else if (mMultipleHeaderAdapter.isFooterView(position)) {
